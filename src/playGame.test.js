...
test("play game with defaults", async (t) => {
  ...
  td.verify(
    res.send(
      `<html><body><h2>Game testGameId1</h2><p><h3>Iteration 1</h3></p><p>Initial text is '${text}'.</p><p>Spoken text (en-IN-Standard-C) is <audio controls><source src="testUrl"></audio></p><p>Heard text is '${text}'</p></body></html>`
    )
  );
  t.end();
});
